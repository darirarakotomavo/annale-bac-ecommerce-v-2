    ...
    const priceDisplay = course.discountPrice
        ? (
            <>
                ...
                <span className="text-2xl font-bold text-emerald-600">{course.discountPrice} €</span>
            </>
        )
        : <span className="text-2xl font-bold text-emerald-600">{course.price} €</span>;

    const phoneNumber = '261322462274';
    const message = `Bonjour, je souhaite m'inscrire au cours : ${course.title} (${course.discountPrice || course.price} €).`;
                        ...
                        <div className="md:col-span-2 space-y-6">
                            ...
                            <Card>
                                <CardBody>
                                    ...
                                    <p className="text-sm text-gray-500 mb-4">{course.lessons.length} leçons • {course.duration}h</p>
                                    <div className="space-y-3">
                                        {course.lessons.map((lesson: any, idx: number) => (
                                            <div key={idx} className="border-b border-gray-100 pb-3 last:border-0 last:pb-0">
                                                <div className="flex items-start justify-between gap-4">
                                                    <div>
                                                        <h4 className="font-medium text-gray-800">{idx + 1}. {lesson.title}</h4>
                                                        {lesson.duration && (
                            ...
                            <Card>
                                <CardBody>
                                    ...
                                    <p className="text-sm text-gray-600">
                                        Professeur licencié en Sciences Physiques, autodidacte en informatique et développement.
                                        Plus de 70 ans d'expérience et de passion pour l'éducation.
                                    </p>
                                </CardBody>
                            </Card>
